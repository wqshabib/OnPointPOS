...
namespace POSSUM.Integration.PaymentDevices.BabsBptiInterop
{
    public class BabsBptiInteropPaymentDevice : IPaymentDevice
    {
        private string paymentDeviceTypeConnectionString;
        private int cookie;
        private volatile BabsBpiInteropApi api;
        ...
        // "babstcp://127.0.0.1:2000"
        public BabsBptiInteropPaymentDevice(string paymentDeviceTypeConnectionString)
        {
        ...
        public bool Connect()
        {
            ...
            runInBackground(() =>
            {
                api = new BabsBpiInteropApi(paymentDeviceTypeConnectionString, dispatcher);
            ...
            runInBackground(() =>
            {
                ...
                switch (transactionType)
                {
                    case PaymentTransactionType.PURCHASE:
                        api.StartTransaction(BabsBpiInteropApi.TransactionTypes.LPP_PURCHASE);
                        break;
                    case PaymentTransactionType.REFUND:
                        api.StartTransaction(BabsBpiInteropApi.TransactionTypes.LPP_REFUND);
                        break;
                    case PaymentTransactionType.REVERSAL:
                        api.StartTransaction(BabsBpiInteropApi.TransactionTypes.LPP_REVERSAL);
                        break;
                    case PaymentTransactionType.BATCH:
                        api.StartTransaction(BabsBpiInteropApi.TransactionTypes.LPP_CLOSEBATCH);
                        break;
                }
            });
        }
