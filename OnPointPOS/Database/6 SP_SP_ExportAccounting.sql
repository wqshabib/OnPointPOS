...
/****** Object:  StoredProcedure [dbo].[SP_ExportAccounting]    Script Date: 2022-09-23 11:11:46 ******/
SET ANSI_NULLS ON
GO
...
DECLARE @LoopCounter INT = 1, @MaxAccountingId INT  , 
        @AccountingId int,@AccountingTax decimal(18,2), @AcNo varchar(50),@Title varchar(100), @SortOrder INT
...
select @Title= Accounting.Name from Accounting where Id=@AccountingId
	...
	INSERT INTO #TempSale (DataType, DataTypeText,SaleDay,DataValue,Amount, SortOrder)
	SELECT @AcNo,@Title,cast(OrderMaster.InvoiceDate as date) as SaleDay,

Cast(Cast((-1)*Sum((((OrderDetail.UnitPrice*OrderDetail.Qty)-OrderDetail.ItemDiscount)/(1+OrderDetail.TaxPercent/100))) as numeric(12,2))as varchar(20)),

(-1)*Sum((((OrderDetail.UnitPrice*OrderDetail.Qty)-OrderDetail.ItemDiscount)/(1+OrderDetail.TaxPercent/100)))
...
WHERE 
OrderMaster.TrainingMode=0 
AND OrderDetail.TaxPercent=@AccountingTax AND 
OrderMaster.Status=13 AND     
OrderMaster.InvoiceGenerated=1 
AND  OrderDetail.Active=1  
AND OrderDetail.ItemType<>1 

AND OrderMaster.InvoiceDate BETWEEN @OpenDate AND @CloseDate   
AND  OrderMaster.TerminalId = @TerminalId
					GROUP BY cast(OrderMaster.InvoiceDate as date),OrderDetail.TaxPercent;
...
--Return  sale
 ...
 select @AcNo=cast(Accounting.AcNo as varchar(10)),@Title= Accounting.Name from Accounting where Id=@AccountingId
	...
	INSERT INTO #TempSale (DataType, DataTypeText,SaleDay,DataValue,Amount, SortOrder)
	SELECT @AcNo,@Title,cast(OrderMaster.InvoiceDate as date) as SaleDay,
Cast(Cast(Sum((((OrderDetail.UnitPrice*OrderDetail.Qty)+OrderDetail.ItemDiscount)/(1+OrderDetail.TaxPercent/100))) as numeric(12,2))as varchar(20)),

(-1)*Sum((((OrderDetail.UnitPrice*OrderDetail.Qty)+OrderDetail.ItemDiscount)/(1+OrderDetail.TaxPercent/100))),@SortOrder
		FROM OrderMaster LEFT JOIN OrderDetail 
			ON OrderMaster.Id = OrderDetail.OrderId 
			inner join Product as Item on Item.Id=OrderDetail.itemID	
