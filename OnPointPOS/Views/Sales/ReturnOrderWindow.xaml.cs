    ...
    public partial class ReturnOrderWindow : Window, IReturnOrderView
    {
        ...
        public void CalculatLeftTotal(IList<OrderLine> list)
        {
            ...
            foreach (var item in list)
            {
                if (item.IsValid)
                {
                    ...
                    OldOrderTotal += (item.Quantity * item.UnitPrice) - item.ItemDiscount;
                    if (item.IngredientItems != null && item.IngredientItems.Count>0)
                        foreach (var ingredient in item.IngredientItems)
                        {
                            ...
                            OldOrderTotal += (ingredient.Quantity * ingredient.UnitPrice) - ingredient.ItemDiscount;// ingredient.GrossAmountDiscounted();
                        }
                }
            }
        ...
        public void CalculatRightTotal(IList<OrderLine> list)
        {
            ...
            foreach (var item in list)
            {
                if (item.IsValid)
                {
                    if (item.Product.ItemType == ItemType.Individual)
                        tax += item.VatAmount();
                    NewOrderTotal += (item.Quantity * item.UnitPrice) - item.ItemDiscount;// item.GrossAmountDiscounted();
                    if (item.IngredientItems != null)
                        foreach (var ingredient in item.IngredientItems)
                        {
                            tax += ingredient.VatAmount();
                            NewOrderTotal += (ingredient.Quantity * ingredient.UnitPrice) - ingredient.ItemDiscount;// ingredient.GrossAmountDiscounted();
                        }
                }
            }
            foreach (var detail in list.Where(p => p.Product.ReceiptMethod == ReceiptMethod.Show_Product_As_Group))
        ...
        private void CheckoutNewButton_Click(object sender, RoutedEventArgs e)
        {
            if (MasterOrder != null && RightOrderLines.Count > 0)
            {
                ...
                Presenter.HandelCheckoutOrderClick(RightOrderTotal);
            }
        }
