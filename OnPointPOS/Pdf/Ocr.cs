...
namespace POSSUM.Pdf
{
    public class Ocr
    {
        public static MemoryStream CreateOld(long intInvoiceID, string strCompanyName, DateTime dtInvoiceTimeStamp, DateTime dtExpireDateTime, string strCustomerReference, string strOcr, long intOrderID, string strPartnerReference, string strOrderText, List<OrderLine> orderLines, string strInvoiceTitle, string strInvoiceText, decimal decTotal, decimal decVATAmount, decimal netTotal, string strReminderText, string strFullName, string strAdressLine1, string strAdressLine2, DateTime dtExpireDate, decimal decReminderFee)
        {
            ...
            try
            {
                source = "invoice.pdf";// System.IO.Path.Combine(Environment.CurrentDirectory, @"\ocr.pdf");
            ...
            {
                source = System.IO.Path.Combine(Environment.CurrentDirectory, @"\invoice.pdf");
            }

            HiQPdf.PdfDocument pdf = HiQPdf.PdfDocument.FromFile(source);
            //Commented by Waqas due to this exception
            //Cannot write the document to output stream. Invalid Serial Number Version.
            //pdf.SerialNumber = Constants.SERIAL;
            ...
            if (File.Exists(Defaults.CompanyInfo.Logo))
            {
                ...
                page.Layout(new HiQPdf.PdfImage(50, 25, 100, Defaults.CompanyInfo.Logo));
            ...
            page.Layout(new HiQPdf.PdfText(350, 140, strCustomerReference, fontStandard));

            // Invoice Id (Projektnr)
            //Refrence code
            Defaults.FakturaReference = string.IsNullOrEmpty(Defaults.FakturaReference) ? Defaults.Outlet.Name : Defaults.FakturaReference;
            page.Layout(new HiQPdf.PdfText(53, 140, Defaults.FakturaReference, fontStandard));
            ...
            if (string.IsNullOrEmpty(strPartnerReference))
                strPartnerReference = " ";
            page.Layout(new HiQPdf.PdfText(53, 185, strPartnerReference, fontStandard));
            ...
            //page.Layout(new HiQPdf.PdfText(230, 228, UI.Sales_UnitPrice+"/"+UI.Sales_Unit, fontheading));
            ...
            foreach (var orderLine in lines)
            {
                ...
                page.Layout(new HiQPdf.PdfText(275, 245 + yPos, orderLine.CartQty, numericfont));
            ...
            page.Layout(new HiQPdf.PdfText(60, 500, Defaults.CompanyInfo.Name, fontSystem));
            page.Layout(new HiQPdf.PdfText(60, 510, Defaults.CompanyInfo.Address1, fontSystem));
            page.Layout(new HiQPdf.PdfText(60, 520, Defaults.CompanyInfo.ZipCode + " " + Defaults.Outlet.City, fontSystem));

            page.Layout(new HiQPdf.PdfText(60 + 100, 500, "Telefon:", fontSystem));
            page.Layout(new HiQPdf.PdfText(60 + 135, 500, Defaults.CompanyInfo.Phone, fontSystem));
            ...
            page.Layout(new HiQPdf.PdfText(60 + 350 + 75, 510, Defaults.CompanyInfo.OrgNo, fontSystem));

            page.Layout(new HiQPdf.PdfText(60 + 350, 520, "Vat-Nummer", fontSystem));
            page.Layout(new HiQPdf.PdfText(60 + 350 + 75, 520, Defaults.CompanyInfo.TaxDescription, fontSystem));
            ...
            */
            // Sender (Consumer)
            page.Layout(new HiQPdf.PdfText(350, 660, strFullName, fontStandard));
            if (string.IsNullOrEmpty(strAdressLine1))
                strAdressLine1 = " ";
            page.Layout(new HiQPdf.PdfText(350, 672, strAdressLine1, fontStandard));
            if (string.IsNullOrEmpty(strAdressLine2))
                strAdressLine2 = " ";
            page.Layout(new HiQPdf.PdfText(350, 690, strAdressLine2, fontStandard));
            ...
            // Bankgiro
            page.Layout(new HiQPdf.PdfText(60, 693, Defaults.CompanyInfo.BankAccountNo, fontStandard));

            // Betalningsmottagare
            page.Layout(new HiQPdf.PdfText(60, 671, Defaults.CompanyInfo.PaymentReceiver, fontStandard));
            ...
            if (orderLines.Count > 15)
            {
                // int pageStart = 15;
                int pagesize = 50;
                int TotalRecord = orderLines.Count;
                int pageNo = 2;
                for (int pageStart = 15; pageStart < TotalRecord;)
                {
                    var oline = orderLines.Skip(pageStart).Take(pagesize).ToList();

                    pdf.AddDocument(WritePage2Old(oline, pageNo));
                    pageNo++;
            ...
            */
            ...
            if (orderLines.Count > index)
            {
                ...
                for (int pageStart = index; pageStart < TotalRecord;)
                {
                    ...
                    pdf.AddDocument(WritePage2(oline, pageNo));
                    pageNo++;
                    pageStart = pageStart + pagesize;
                }
            }
        ...
        public static HiQPdf.PdfDocument WritePage2Old(List<OrderLine> orderLines, int pageNo)
        {

            string source = string.Empty;
            try
            {
                source = "invoicepage2.pdf";// System.IO.Path.Combine(Environment.CurrentDirectory, @"\ocr.pdf");
                //source = System.IO.Path.Combine(Environment.CurrentDirectory, @"..\..\Pdf\invoice.pdf");
            }
            catch { }
            ...
            foreach (var orderLine in orderLines)
            {
                ...
                page.Layout(new HiQPdf.PdfText(265, 60 + yPos, orderLine.CartQty, numericfont));
                string unitPrice = Text.AlignNumber(orderLine.UnitPrice, decTotal);
        ...
        public static HiQPdf.PdfDocument WritePage2(List<OrderLineViewModel> orderLines, int pageNo)
        {
