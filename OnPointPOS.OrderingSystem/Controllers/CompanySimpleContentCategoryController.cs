...
{
    public class CompanySimpleContentCategoryController : ApiController
    {
        ...
        public CompanySimpleContentCategoryController()
        {
            _mSimpleContentCategoryLists.Add(_mSimpleContentCategoryList);
        }

        public HttpResponseMessage Get(string secret, string companyId, string contentParentCategoryId)
        {
            if (string.IsNullOrEmpty(secret) && !ML.Common.Text.IsGuidNotEmpty(companyId) && !ML.Common.Text.IsGuidNotEmpty(contentParentCategoryId))
            {
            ...
            IQueryable<DB.tContentCategory> contentCategories = new DB.ContentCategoryRepository().GetActiveByContentParentCategoryGuid(new Guid(contentParentCategoryId));
