...
{
    public class OrderSearchStatController : ApiController
    {
        List<OrderStat> _mOrderStats = new List<OrderStat>();
        OrderStat _mOrderStat = new OrderStat();

        public OrderSearchStatController()
        {
        ...
        public HttpResponseMessage Post(string token, string id)
        {
            ...
            {
                orders = new DB.OrderRepository().SearchOrders(usertoken.CompanyGuid, Guid.Empty, 1000000000, dtStartDateTime, dtEndDateTime, strSearch, DB.OrderStatusRepository.OrderStatus.Completed);
            }
            else
            {
                orders = new DB.OrderRepository().SearchOrders(usertoken.CompanyGuid, orderPrinters.FirstOrDefault().OrderPrinterGuid, 1000000000, dtStartDateTime, dtEndDateTime, strSearch, DB.OrderStatusRepository.OrderStatus.Completed);
            }
