...
{
    public class FNReportController : MyBaseController
    { 
        // GET: Sales
        ...
        public ActionResult OrderList(string dtFrom, string dtTo, Guid  terminal)
        {
            ...
            try
            {
                ...
                {
                    string query = @"SELECT      OrderMaster.FnResponse,  OrderMaster.Id, Receipt.ReceiptNumber, OrderMaster.OrderNoOfDay,OrderMaster.Comments, OrderMaster.InvoiceDate, OrderMaster.OrderTotal, OrderMaster.InvoiceGenerated
FROM            OrderMaster INNER JOIN
                    ...
                    using (var conn = db.Database.Connection)
                    {
                        ...
                        while (dr.Read())
                        {
                            orders.Add(new OrderViewModel
                            {
                                ...
                                FnResponse = Convert.ToString(dr["FnResponse"])
                            });
                        }
                        dr.Dispose();
                    }
                    ...
                    foreach (var item in orders)
                    {
                        ...
                        if (!string.IsNullOrEmpty(item.FnResponse))
                        {
                            try
                            {
                                var voucer = JsonConvert.DeserializeObject<FNResponse>(item.FnResponse);
                                if (voucer != null)
                                {
                                    item.FnId = "https://apps5.fortnox.se/app/5351b81cf92f48c69159e6d1cb92978c/bf/voucher/" + voucer.Voucher.VoucherSeries + "-" + voucer.Voucher.VoucherNumber;
                                }
                            }
                            catch (Exception)
                            {
        ...
        public ActionResult OrderReceipt(Guid id)
        {
            ...
            using (var db = GetConnection)
            {
                ...
                if (orderMaster != null)
                {
                    //  var outlet = outletRepo.FirstOrDefault(o => o.Id == orderMaster.Outlet.Id);
